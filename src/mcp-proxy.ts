...
export const createServer = async () => {
  ...
  // Call Tool Handler with Smart Selection and improved error handling
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    const { name, arguments: args } = request.params;
    
    // First, try to find the server that owns this tool
    let clientForTool = toolToClientMap.get(name);
    
    if (!clientForTool) {
      // 如果工具不在映射中，使用智能选择找到最佳服务器
      const selectionResult = selectBestServer(request, getHealthyClients());
      
      if (selectionResult.selectedServer) {
        clientForTool = connectedClients.find(client => client.name === selectionResult.selectedServer);
        console.log(`Smart selection chose ${selectionResult.selectedServer} for tool ${name} (confidence: ${selectionResult.confidence.toFixed(2)})`);
      }
    }

    if (!clientForTool) {
      throw new Error(`No available server for tool: ${name}`);
    }
  ...
  server.setRequestHandler(GetPromptRequestSchema, async (request) => {
    ...
    if (!clientForPrompt) {
      // Use smart selection if not found in map
      const selectionResult = selectBestServer(request, getHealthyClients());
      if (selectionResult.selectedServer) {
        clientForPrompt = connectedClients.find(client => client.name === selectionResult.selectedServer);
      }
    }
  ...
  server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
    ...
    if (!clientForResource) {
      ...
      const selectionResult = selectBestServer(request, getHealthyClients());
      if (selectionResult.selectedServer) {
        clientForResource = connectedClients.find(client => client.name === selectionResult.selectedServer);
      }
    }
